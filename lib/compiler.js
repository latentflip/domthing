...
function precompileTextNode(element, o) {
    ...
    return [
        ...
        indent(o.depth + 1, ");"),
        indent(o.depth + 1, "node.data = expr.value ? expr.value : '';"),
        indent(o.depth + 1, "expr.on('change', function (text) { node.data = text ? text : ''; });"),
...
function precompileElement(element, o) {
    ...
    Object.keys(element.attributes).forEach(function (attrName) {
        ...
        if (isBooleanAttribute(attrName)) {
            ...
            body = body.concat([
                indent(o.depth, "element.setAttribute('" + attrName + "', expr.value ? runtime.helpers.ESCAPE_FOR_ATTRIBUTE('" + attrName + "', expr.value) : '');"),
                indent(o.depth, "expr.on('change', function (v) {"),
                indent(o.depth + 1, "element.setAttribute('" + attrName + "', v ? runtime.helpers.ESCAPE_FOR_ATTRIBUTE('" + attrName + "', v) : '');"),
                indent(o.depth, "});")
            ]);
        }
        return;
