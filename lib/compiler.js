...
Compiler.prototype.compileExpression = function (ast) {
    if (ast.type === 'Literal') {
        this.write(
            "runtime.hooks.EVENTIFY_LITERAL.call(template, " + JSON.stringify(ast.value) + ")"
        );
    }

    if (ast.type === 'Binding') {
        this.write(
            "runtime.hooks.EVENTIFY_BINDING.call(template, context, '" + ast.keypath + "')"
        );
    }
...
Compiler.prototype.compileStandardAttribute = function (attrName) {
    this.write(
        "element.setAttribute('" + attrName + "', expr.value ? runtime.hooks.ESCAPE_FOR_ATTRIBUTE('" + attrName + "', expr.value) : '');",
        "expr.on('change', function (v) {",
        "  element.setAttribute('" + attrName + "', v ? runtime.hooks.ESCAPE_FOR_ATTRIBUTE('" + attrName + "', v) : '');",
        "});"
    );
};
